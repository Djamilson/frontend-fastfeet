...
export default function DeliverymanForm() {
    ...
    async function searchDeliveryman() {
      if (typeof deliverymanId !== typeof undefined) {
        try {
          ...
          setImage({
            preview: `${res.data.person.avatar.url}-sm`,
            file: '',
            id_file: '',
          });
  ...
  async function handleSubmit(data) {
    try {
      ...
      const { person, address, password } = data;
      ...
      if (typeof deliverymanSelect.id !== typeof undefined) {
        ...
        if (citySelect.value === 0) {
          setLoading(false);
          toast.error('Atenção, tem que selecionar uma cidade!');
          return;
        }

        if (password === '') {
          setLoading(false);
          toast.error('Atenção, tem que adicionar uma senha!');
          return;
        }
        ...
        <Container>
          ...
          <Form ref={formRef} onSubmit={handleSubmit}>
            ...
            <Content>
              ...
              <hr />
              <div>
                <label htmlFor="idPassword">Senha para acesso</label>
                <Input
                  id="idPassword"
                  type="password"
                  name="password"
